...
**状态码**:
- `200 OK`: 成功
- `500 Internal Server Error`: MLflow 连接失败
...
result = single_prediction(sample)
...
async function predict(data) {
  const response = await fetch('http://localhost:8005/prediction_api', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
- **单次预测**: < 100ms (P95)
- **批次预测**: ~ 10-50ms per sample
- **模型加载**: ~ 1-2s (首次，之后使用缓存)

### 吞吐量

- **单次预测**: ~ 100 QPS (单实例)
- **批次预测**: ~ 1000 samples/sec
...
```python
# NOT RECOMMENDED: Loop calling single prediction
for sample in samples:
    result = single_prediction(sample)

# RECOMMENDED: Use batch prediction
df = pd.DataFrame(samples)
df.to_csv('batch.csv', index=False)
results = batch_prediction('batch.csv')
```
...
async def predict_async(session, data):
    async with session.post(f"{BASE_URL}/prediction_api", json=data) as response:
        return await response.json()

async def main():
    async with aiohttp.ClientSession() as session:
        tasks = [predict_async(session, sample) for sample in samples]
...
def robust_prediction(data):
    response = requests.post(f"{BASE_URL}/prediction_api", json=data)
...
- 查看 [监控文档](MONITORING.md) 了解 API 性能监控
